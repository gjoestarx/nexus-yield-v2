...
export function GrowthCurve({ simulation, capital }: GrowthCurveProps) {
 ...
 <div className="h-[300px]">
  ...
  <Tooltip content={({ payload }) => {
   ...
   <div className="text-sm text-[var(--positive)]">Best: {formatUsd(d.best)}</div>
   <div className="text-sm text-[var(--accent)]">Expected: {formatUsd(d.value)}</div>
   <div className="text-sm text-[var(--negative)]">Worst: {formatUsd(d.worst)}</div>
    </div>
   );
  }} />
 ...
 <div className="mt-3 flex justify-center gap-6 text-xs text-[var(--text-3)]">
  <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-[var(--positive)]" /> Best case</span>
  <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-[var(--accent)]" /> Expected</span>
  <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-[var(--negative)]" /> Worst case</span>
 </div>
  </Card>
 );
}
