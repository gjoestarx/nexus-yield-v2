  ...
  return (
    <Card className="panel-glass">
      ...
      <div className="h-[300px]">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 10, right: 10, bottom: 10, left: 10 }}>
            <defs>
              <linearGradient id="growthGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#06b6d4" stopOpacity={0.3} /><stop offset="95%" stopColor="#06b6d4" stopOpacity={0} /></linearGradient>
            ...
            <XAxis dataKey="day" tick={{ fontSize: 11, fill: '#4a5f82' }} label={{ value: 'Days', position: 'insideBottom', offset: -5, style: { fontSize: 11, fill: '#4a5f82' } }} />
            <YAxis tick={{ fontSize: 11, fill: '#4a5f82' }} tickFormatter={(v) => `$${(v / 1000).toFixed(1)}K`} />
            <Tooltip content={({ payload }) => {
              ...
              return (
                <div className="panel-glass rounded-xl px-4 py-3 shadow-xl">
                  ...
                  <div className="text-sm text-[var(--cyan)]">Expected: {formatUsd(d.value)}</div>
                  <div className="text-sm text-[var(--red)]">Worst: {formatUsd(d.worst)}</div>
                </div>
              );
            }} />
            <ReferenceLine y={capital} stroke="#4a5f82" strokeDasharray="5 5" label={{ value: 'Initial Capital', position: 'right', style: { fontSize: 11, fill: '#4a5f82' } }} />
            <Area type="monotone" dataKey="best" stroke="#22c55e" strokeWidth={1} fill="url(#bestGrad)" strokeDasharray="4 4" />
            <Area type="monotone" dataKey="value" stroke="#06b6d4" strokeWidth={2} fill="url(#growthGrad)" />
      ...
      <div className="mt-3 flex justify-center gap-6 text-xs text-[var(--text-muted)]">
        ...
        <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-[var(--cyan)]" /> Expected</span>
