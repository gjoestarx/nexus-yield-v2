...
export function PoolHeatmap({ pools, risks }: PoolHeatmapProps) {
 ...
 <div className="border-b border-[var(--border-subtle)] px-5 py-3">
  ...
  <div className="text-[10px] text-[var(--text-3)]">Color = APY/Risk ratio · Size = TVL · {sortedCells.length} pools</div>
 </div>
 <div className="flex items-center justify-between px-5 py-2 border-b border-[var(--border-subtle)]/50">
 ...
 <div className="relative p-5">
  ...
  {hoveredCell && (
   <div className="surface pointer-events-none fixed z-[100] rounded-[var(--radius-md)] px-4 py-3 shadow-xl" style={{ left: tooltipPos.x, top: tooltipPos.y - 8, transform: 'translate(-50%, -100%)' }}>
  <div className="text-sm font-bold">{hoveredCell.pool.symbol}</div>
  <div className="text-[10px] text-[var(--text-3)]">{hoveredCell.pool.protocol} · {CHAIN_LABELS[hoveredCell.pool.chain]}</div>
  <div className="mt-2 space-y-0.5 text-xs">
   <div>APY: <span className="font-mono text-[var(--positive)]">{formatPct(hoveredCell.pool.apy)}</span></div>
   <div>Risk: <span className="font-mono">{hoveredCell.risk?.score ?? '—'}</span>{hoveredCell.risk && <span className="ml-1 text-[var(--text-3)]">({hoveredCell.risk.category})</span>}</div>
 ...
 {summary && (
  <div className="grid grid-cols-3 gap-px border-t border-[var(--border-subtle)]/50">
   <div className="bg-white/[0.02] px-4 py-2 text-center"><div className="text-[9px] font-medium uppercase tracking-wider text-[var(--text-3)]">Best Ratio</div><div className="mt-0.5 text-xs font-mono font-semibold text-[var(--positive)]">{summary.bestCell.pool.symbol} ({summary.bestCell.ratio.toFixed(3)})</div></div>
   <div className="bg-white/[0.02] px-4 py-2 text-center"><div className="text-[9px] font-medium uppercase tracking-wider text-[var(--text-3)]">Avg Ratio</div><div className="mt-0.5 text-xs font-mono font-semibold">{summary.avgRatio.toFixed(3)}</div></div>
   <div className="bg-white/[0.02] px-4 py-2 text-center"><div className="text-[9px] font-medium uppercase tracking-wider text-[var(--text-3)]">Worst Ratio</div><div className="mt-0.5 text-xs font-mono font-semibold text-[var(--negative)]">{summary.worstCell.pool.symbol} ({summary.worstCell.ratio.toFixed(3)})</div></div>
  </div>
 )}
  </Card>
 );
