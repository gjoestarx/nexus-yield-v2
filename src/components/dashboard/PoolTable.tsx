  ...
  return (
    <div className="space-y-2">
      ...
      <Card className="panel-glass overflow-hidden p-0">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[var(--border)] bg-black/20">
                <th className="px-4 py-3 text-left text-[11px] font-medium uppercase tracking-wider text-[var(--text-muted)]">Pool</th>
                <th className="px-4 py-3 text-left text-[11px] font-medium uppercase tracking-wider text-[var(--text-muted)]">Chain</th>
                <th className="px-4 py-3 text-right text-[11px] font-medium uppercase tracking-wider text-[var(--text-muted)]">TVL</th>
                <th className="px-4 py-3 text-right text-[11px] font-medium uppercase tracking-wider text-[var(--text-muted)]">APY</th>
                <th className="px-4 py-3 text-right text-[11px] font-medium uppercase tracking-wider text-[var(--text-muted)]">30d Avg</th>
                <th className="px-4 py-3 text-center text-[11px] font-medium uppercase tracking-wider text-[var(--text-muted)]">Risk</th>
                <th className="px-4 py-3 text-center text-[11px] font-medium uppercase tracking-wider text-[var(--text-muted)]">Stability</th>
                <th className="px-4 py-3 text-center text-[11px] font-medium uppercase tracking-wider text-[var(--text-muted)]">Score</th>
            ...
            <tbody>
              {pools.map((pool) => {
                ...
                return (
                  <tr key={pool.id} onClick={() => onSelect(pool)} className="cursor-pointer border-b border-[var(--border)]/30 transition-colors hover:bg-white/5">
                    <td className="px-4 py-2.5"><div className="font-medium">{pool.symbol}</div><div className="text-[10px] text-[var(--text-muted)]">{pool.protocol}</div></td>
        ...
        {pools.length === 0 && <div className="p-8 text-center text-[var(--text-muted)]">No pools match your filters.</div>}
      </Card>
    </div>
  );
}
