...
export function FiltersBar({ filters, onUpdate, onRefresh, loading }: FiltersBarProps) {
 ...
 <div className="flex gap-1 rounded-[var(--radius-sm)] bg-[var(--bg-1)] p-1">
  ...
  className={`rounded-md px-3 py-1.5 text-sm font-medium transition-colors ${filters.mode === m.value ? 'bg-[var(--accent-muted)] text-[var(--accent)] shadow-sm' : 'text-[var(--text-3)] hover:text-[var(--text-1)]'}`}>
  {m.emoji} {m.label}
   </button>
  ))}
 </div>
 <div className="flex gap-2">
  ...
  className={`rounded-full px-3 py-1 text-xs font-medium transition-colors ${filters.chains.includes(chain) ? 'bg-[var(--accent)] text-white' : 'bg-[var(--bg-1)] text-[var(--text-3)] hover:text-[var(--text-1)]'}`}>
  {CHAIN_LABELS[chain]}
   </button>
  ))}
 </div>
 <div className="flex items-center gap-2">
  <span className="text-sm text-[var(--text-3)]">Min TVL:</span>
  <select value={filters.minTvl} onChange={(e) => onUpdate({ minTvl: Number(e.target.value) })}
   className="rounded-md border border-[var(--border-subtle)] bg-[var(--bg-1)] px-2 py-1 text-sm text-white">
