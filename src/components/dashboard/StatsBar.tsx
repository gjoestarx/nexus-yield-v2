...
export function StatsBar({ stats, loading }: StatsBarProps) {
  if (loading || !stats) {
    return (
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        {[...Array(4)].map((_, i) => (<Card key={i} className="panel-glass animate-pulse"><div className="h-4 w-24 rounded bg-white/5" /><div className="mt-2 h-8 w-16 rounded bg-white/5" /></Card>))}
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
      {items.map((item) => (
        <Card key={item.label} className="panel-glass" style={{ borderLeft: `3px solid ${item.color}` }}>
