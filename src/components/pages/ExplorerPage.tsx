...
export function ExplorerPage({ pools, risks, rankings, onSelectPool }: ExplorerPageProps) {
 ...
 const SortHeader = ({ k, label }: { k: SortKey; label: string }) => (
  <th className="cursor-pointer px-4 py-3 text-left text-[11px] font-medium uppercase tracking-wider text-[var(--text-3)] transition-colors hover:text-white" onClick={() => toggleSort(k)}>
 ...
 <div className="surface flex flex-wrap items-center gap-3 p-4">
  ...
  className="w-full rounded-[var(--radius-sm)] border border-[var(--border-subtle)] bg-black/30 py-2 pl-9 pr-4 text-sm text-white placeholder-[var(--text-3)] outline-none focus:border-[var(--accent)]/30 focus:ring-1 focus:ring-[var(--accent)]/30" />
  </div>
  <select value={chainFilter} onChange={(e) => setChainFilter(e.target.value as Chain | 'all')}
   className="rounded-[var(--radius-sm)] border border-[var(--border-subtle)] bg-black/30 px-3 py-2 text-sm text-white">
  ...
  <div className="text-xs text-[var(--text-3)]">{filtered.length} pools</div>
 </div>
 ...
 <Card className="surface p-0 overflow-hidden">
  ...
  <thead>
   <tr className="border-b border-[var(--border-subtle)] bg-black/20">
    <th className="px-4 py-3 text-left text-[11px] font-medium uppercase tracking-wider text-[var(--text-3)]">#</th>
  ...
  <tbody>
   {filtered.map((pool, i) => {
    ...
    return (
   <tr key={pool.id} onClick={() => onSelectPool(pool)} className="cursor-pointer border-b border-[var(--border-subtle)]/30 transition-colors hover:bg-white/5">
    <td className="px-4 py-2.5 text-xs text-[var(--text-3)]">{i + 1}</td>
    <td className="px-4 py-2.5">
     <div className="font-medium">{pool.symbol}</div>
     <div className="text-[10px] text-[var(--text-3)]">{pool.protocol}</div>
    </td>
    <td className="px-4 py-2.5">
    ...
    <td className="px-4 py-2.5 font-mono text-xs font-semibold text-[var(--positive)]">{formatPct(pool.apy)}</td>
  ...
  {filtered.length === 0 && <div className="p-12 text-center text-[var(--text-3)]">No pools match your search.</div>}
 </Card>
  </div>
 );
}
