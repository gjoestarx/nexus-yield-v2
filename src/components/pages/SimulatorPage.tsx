    ...
    <div className="space-y-6 animate-in">
              ...
              return (
                <div key={pool.id} onClick={() => setSelectedPoolId(pool.id)}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {risk && <span className="text-[var(--text-muted)]">R:{risk.score}</span>}
                  </div>
                </div>
              );
            })}
      ...
      {showSaved && (
        <Card className="card-glass p-0 overflow-hidden animate-in">
          ...
          {savedSims.length === 0 ? (
            <div className="flex h-[120px] items-center justify-center text-xs text-[var(--text-muted)]">No saved simulations yet.</div>
          ) : (
              ...
              {savedSims.map((sim) => (
                <div key={sim.id} className="flex items-center justify-between px-5 py-3 hover:bg-white/[0.02] transition-colors">
                  <div>
                    ...
                    <div className="text-[10px] text-[var(--text-muted)]">${sim.capital.toLocaleString()} · {sim.days}d · {sim.riskMode}</div>
                  </div>
                  <div className="flex items-center gap-4">
                    <div className="text-right">
                      <div className="text-xs font-mono text-[var(--green)]">{formatUsd(sim.results.expectedReturn)}</div>
