...
export function WatchlistPage({ pools, risks }: WatchlistPageProps) {
 ...
 {isConnected ? (
  <div className="surface flex items-center gap-3 bg-[var(--positive-muted)] p-3">
   <div className="h-2 w-2 rounded-full bg-[var(--positive)]" />
   <div className="text-[11px]"><span className="text-[var(--positive)]">Wallet connected</span><span className="text-[var(--text-3)]"> — watchlist saved to your wallet address</span></div>
  </div>
 ) : (
  <div className="surface flex items-center gap-3 bg-[var(--warning-muted)] p-3">
   <div className="h-2 w-2 rounded-full bg-[var(--warning)]" />
   <div className="text-[11px]"><span className="text-[var(--warning)]">No wallet connected</span><span className="text-[var(--text-3)]"> — watchlist saved locally. Connect wallet to persist across devices.</span></div>
  </div>
 )}

 {summary && (
  <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
   <div className="surface p-4" style={{ borderLeft: '3px solid var(--accent)' }}><div className="text-[10px] font-medium uppercase tracking-wider text-[var(--text-3)]">Watching</div><div className="mt-1 text-[22px] font-bold text-[var(--accent)]">{summary.count}</div></div>
   <div className="surface p-4" style={{ borderLeft: '3px solid var(--positive)' }}><div className="text-[10px] font-medium uppercase tracking-wider text-[var(--text-3)]">Avg APY</div><div className="mt-1 text-[22px] font-bold text-[var(--positive)]">{formatPct(summary.avgApy)}</div></div>
   <div className="surface p-4" style={{ borderLeft: '3px solid var(--warning)' }}><div className="text-[10px] font-medium uppercase tracking-wider text-[var(--text-3)]">Avg Risk</div><div className="mt-1 text-[22px] font-bold text-[var(--warning)]">{summary.avgRisk.toFixed(1)}</div></div>
   <div className="surface p-4" style={{ borderLeft: '3px solid var(--warning)' }}><div className="text-[10px] font-medium uppercase tracking-wider text-[var(--text-3)]">Total TVL</div><div className="mt-1 text-[22px] font-bold text-[var(--warning)]">{formatUsd(summary.totalTvl)}</div></div>
  </div>
 )}
 ...
 <div className="surface p-4">
  ...
  {searchResults.map((pool) => (
   <div key={pool.id} onClick={() => { toggleWatch(pool.id); setSearch(''); }} className="flex cursor-pointer items-center justify-between rounded-[var(--radius-sm)] px-3 py-2 text-[11px] hover:bg-white/[0.03]">
    ...
    <div className="flex items-center gap-3"><span className="text-[var(--positive)]">{formatPct(pool.apy)}</span><span className="text-[var(--accent)]">+ Watch</span></div>
   </div>
  ))}
   </div>
  )}
 ...
 <div className="surface p-0 overflow-hidden">
  ...
  {(['apy', 'risk', 'tvl'] as const).map((s) => (
   <button key={s} onClick={() => setSortBy(s)} className={`rounded-md px-2 py-1 text-[10px] font-medium transition-all ${sortBy === s ? 'bg-[var(--accent-muted)] text-[var(--accent)]' : 'text-[var(--text-3)] hover:text-[var(--text-1)]'}`}>{s.toUpperCase()}</button>
  ))}
   </div>
  </div>
  {watchedPools.length === 0 ? (
   <div className="flex h-[200px] items-center justify-center text-[12px] text-[var(--text-3)]">No pools in watchlist. Search above to add.</div>
  ) : (
  ...
  {watchedPools.map((pool) => {
   ...
   <div className="flex items-center gap-3">
    <button onClick={() => toggleWatch(pool.id)} className="text-[var(--warning)] hover:text-[var(--negative)] text-[14px]">★</button>
    <div className="h-2 w-2 rounded-full" style={{ background: CHAIN_COLORS[pool.chain] }} />
    <div><div className="text-[12px] font-medium">{pool.symbol}</div><div className="text-[10px] text-[var(--text-3)]">{pool.protocol} · {CHAIN_LABELS[pool.chain]}</div></div>
   </div>
   <div className="flex items-center gap-6">
    <div className="text-right"><div className="text-[13px] font-bold font-mono text-[var(--positive)]">{formatPct(pool.apy)}</div><div className="text-[9px] text-[var(--text-3)]">30d: {formatPct(pool.apyMean30d)}</div></div>
    <div className="text-right"><div className="text-[12px] font-mono">{formatUsd(pool.tvlUsd)}</div><div className="text-[9px] text-[var(--text-3)]">TVL</div></div>
    <div className="text-right"><div className={`text-[12px] font-mono font-semibold ${(risk?.score ?? 0) < 30 ? 'text-[var(--positive)]' : (risk?.score ?? 0) < 60 ? 'text-[var(--warning)]' : 'text-[var(--negative)]'}`}>{risk?.score ?? '—'}</div><div className="text-[9px] text-[var(--text-3)]">Risk</div></div>
    {pool.url && (<a href={pool.url} target="_blank" rel="noopener noreferrer" className="rounded-md bg-[var(--accent-muted)] px-2 py-1 text-[10px] text-[var(--accent)] hover:bg-[var(--accent)]/20">↗</a>)}
   </div>
    </div>
   );
  })}
