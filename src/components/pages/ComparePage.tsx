...
export function ComparePage({ pools, risks }: ComparePageProps) {
 ...
 <div className="surface p-4">
  <div className="mb-3 flex items-center justify-between">
   ...
   <div className="text-[10px] text-[var(--text-3)]">{selectedIds.length}/4 selected</div>
  </div>
  ...
  {searchResults.length > 0 && (
   <div className="absolute left-0 right-0 top-full z-10 mt-1 max-h-[250px] overflow-y-auto rounded-[var(--radius-sm)] border border-[var(--border-subtle)] bg-[var(--bg-2)] shadow-xl">
   ...
   <div key={pool.id} onClick={() => addPool(pool.id)} className="flex cursor-pointer items-center justify-between px-3 py-2 text-[11px] hover:bg-white/[0.03]">
    <div><span className="font-medium">{pool.symbol}</span><span className="ml-2 text-[var(--text-3)]">{pool.protocol} · {CHAIN_LABELS[pool.chain]}</span></div>
  ...
  {selectedPools.map((pool, i) => (
   <div key={pool.id} className="flex items-center gap-2 rounded-[var(--radius-sm)] px-3 py-1.5 text-[11px]" style={{ background: `${COLORS[i]}15`, border: `1px solid ${COLORS[i]}30` }}>
    <div className="h-2 w-2 rounded-full" style={{ background: COLORS[i] }} />
    <span className="font-medium">{pool.symbol}</span>
    <span className="text-[var(--text-3)]">{pool.protocol}</span>
    <button onClick={() => removePool(pool.id)} className="ml-1 text-[var(--text-3)] hover:text-[var(--negative)]">✕</button>
   </div>
  ))}
   </div>
  )}
 ...
 {selectedPools.length >= 2 && (
  <div className="surface p-0 overflow-hidden">
   <div className="border-b border-[var(--border-subtle)] px-5 py-3"><div className="text-[12px] font-semibold">Side-by-Side Comparison</div></div>
   <div className="overflow-x-auto">
  <table className="w-full text-[12px]">
   <thead>
    <tr className="border-b border-[var(--border-subtle)]">
   <th className="px-4 py-2.5 text-left text-[10px] font-medium uppercase tracking-wider text-[var(--text-3)]">Metric</th>
   ...
   <tbody className="divide-y divide-[var(--border-subtle)]">
    <tr><td className="px-4 py-2 text-[var(--text-3)]">Chain</td>{selectedPools.map((pool) => (<td key={pool.id} className="px-4 py-2 text-right"><span style={{ color: CHAIN_COLORS[pool.chain] }}>{CHAIN_LABELS[pool.chain]}</span></td>))}</tr>
    <tr><td className="px-4 py-2 text-[var(--text-3)]">Protocol</td>{selectedPools.map((pool) => (<td key={pool.id} className="px-4 py-2 text-right">{pool.protocol}</td>))}</tr>
    {metrics.map((m) => (
   <tr key={m.label}>
    <td className="px-4 py-2 text-[var(--text-3)]">{m.label}</td>
    ...
    if (selectedPools.length >= 2) { const vals = selectedPools.map((p) => (p as unknown as Record<string, unknown>)[m.key] as number); const best = m.higher ? Math.max(...vals) : Math.min(...vals); isBest = raw === best; }
     ...
     return (<td key={pool.id} className={`px-4 py-2 text-right font-mono ${isBest ? 'font-semibold text-[var(--positive)]' : ''}`}>{value} {isBest && '✓'}</td>);
    })}
   </tr>
    ))}
   </tbody>
 ...
 {selectedPools.length >= 2 && chartData.length > 0 && (
  ...
  <div className="text-[10px] text-[var(--text-3)]">{loadingHistory ? 'Loading history...' : `${chartData.length} data points`}</div>
   </div>
  ...
  <ResponsiveContainer width="100%" height="100%">
   ...
   <div className="flex justify-center gap-4 border-t border-[var(--border-subtle)] py-2">
 ...
 {selectedPools.length < 2 && (
  <div className="surface flex h-[200px] items-center justify-center">
   <div className="text-center text-[var(--text-3)]"><div className="text-2xl opacity-30">⇄</div><div className="mt-2 text-[12px]">Select at least 2 pools to compare</div></div>
  </div>
 )}
  </div>
 );
