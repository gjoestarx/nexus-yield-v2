...
export function PortfolioBuilder({ pools, risks }: PortfolioBuilderProps) {
 ...
 <div className="grid gap-4 lg:grid-cols-3">
  ...
  <input type="text" placeholder="Search pools..." value={search} onChange={(e) => setSearch(e.target.value)}
   className="w-full rounded-[var(--radius-sm)] border border-[var(--border-subtle)] bg-[var(--bg-1)] py-2 pl-9 pr-3 text-[12px] text-[var(--text-0)] outline-none focus:border-[var(--accent)]/30" />
   </div>
   {searchResults.length > 0 && (
  <div className="max-h-[300px] space-y-0.5 overflow-y-auto">
   {searchResults.map((pool) => (
    <div key={pool.id} onClick={() => addPool(pool.id)} className="flex cursor-pointer items-center justify-between rounded-[var(--radius-sm)] px-3 py-2 text-[11px] hover:bg-white/[0.03]">
   <div><div className="font-medium">{pool.symbol}</div><div className="text-[var(--text-3)]">{pool.protocol}</div></div>
   <div className="text-[var(--positive)]">{formatPct(pool.apy)}</div>
    </div>
   ))}
  </div>
   )}
  ...
  <div>
   <label className="mb-1 block text-[10px] text-[var(--text-3)]">Capital (USD)</label>
   <input type="number" value={capital} onChange={(e) => setCapital(Number(e.target.value))}
    className="w-full rounded-[var(--radius-sm)] border border-[var(--border-subtle)] bg-[var(--bg-1)] px-3 py-2 text-[12px] text-[var(--text-0)] outline-none" />
  </div>
  <div>
   <label className="mb-1 block text-[10px] text-[var(--text-3)]">Days</label>
   <input type="range" min={7} max={730} value={days} onChange={(e) => setDays(Number(e.target.value))} className="w-full accent-[var(--accent)]" />
   <div className="text-[10px] text-[var(--text-3)]">{days} days</div>
  </div>
   </div>
  </div>
  ...
  <div className={`text-[11px] font-mono ${totalWeight === 100 ? 'text-[var(--positive)]' : totalWeight > 100 ? 'text-[var(--negative)]' : 'text-[var(--warning)]'}`}>{totalWeight}% / 100%</div>
   </div>
   {allocations.length === 0 ? (
  <div className="flex h-[200px] items-center justify-center text-[12px] text-[var(--text-3)]">Search and add pools above</div>
   ) : (
  <div className="space-y-2">
   {allocations.map((a, i) => {
    ...
    return (
   <div key={a.poolId} className="flex items-center gap-3 rounded-[var(--radius-sm)] bg-white/[0.02] p-2.5">
    ...
    <div className="flex-1 min-w-0">
     ...
     <div className="text-[9px] text-[var(--text-3)]">{pool?.protocol} · {formatPct(pool?.apy ?? 0)} APY · Risk:{risk?.score}</div>
    </div>
    <input type="number" value={a.weight} onChange={(e) => updateWeight(a.poolId, Number(e.target.value))}
     className="w-14 rounded border border-[var(--border-subtle)] bg-transparent px-2 py-1 text-center text-[11px] font-mono outline-none" />
    <span className="text-[10px] text-[var(--text-3)]">%</span>
    <button onClick={() => removePool(a.poolId)} className="text-[var(--text-3)] hover:text-[var(--negative)] text-[11px]">✕</button>
   </div>
    );
   })}
  </div>
  ...
  <>
   ...
   <div className="mt-2 flex flex-wrap justify-center gap-2">
    {pieData.map((d) => (
   <div key={d.name} className="flex items-center gap-1 text-[9px] text-[var(--text-3)]">
  ...
  <div className="flex h-[200px] items-center justify-center text-[11px] text-[var(--text-3)]">Add pools to see allocation</div>
   )}
  </div>
 </div>
 ...
 {simulation && (
  ...
  <MiniStat label="Weighted APY" value={formatPct(simulation.weightedApy)} color="var(--positive)" />
  <MiniStat label="Weighted Risk" value={simulation.weightedRisk.toFixed(1)} color="var(--warning)" />
  <MiniStat label="Expected Return" value={formatUsd(simulation.expectedReturn)} color="var(--accent)" />
  <MiniStat label="Net Profit" value={formatUsd(simulation.netReturn)} color="var(--positive)" />
  <MiniStat label="Diversification" value={`${(simulation.diversification * 100).toFixed(0)}%`} color="var(--warning)" />
   </div>
   <div className="surface p-0 overflow-hidden">
  <div className="border-b border-[var(--border-subtle)] px-5 py-3">
   <div className="text-[12px] font-semibold">Portfolio Growth Projection</div>
   <div className="text-[10px] text-[var(--text-3)]">{formatUsd(capital)} → {formatUsd(simulation.expectedReturn)} in {days} days</div>
  </div>
...
function MiniStat({ label, value, color }: { label: string; value: string; color: string }) {
 return (
  <div className="surface p-3" style={{ borderLeft: `3px solid ${color}` }}>
 <div className="text-[9px] font-medium uppercase tracking-wider text-[var(--text-3)]">{label}</div>
